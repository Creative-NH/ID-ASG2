...
$(document).ready(function () {
    ...
    $("#contact-submit").on("click", function (e) {
      ...
      let contactNum = $("#contact-msg").val();
      if (contactName !=""){
      var player = document.querySelector("lottie-player");
  player.load("https://assets8.lottiefiles.com/packages/lf20_jbrw3hcz.json")}
      ...
      $.ajax(settings).done(function (response) {
        ...
        for (var i = 0; i < response.length && i < limit; i++) {
          ...
          */
          ...
          content = `${content}<tr id='${response[i]._role}'><td>${response[i].name}</td>
          <td>${response[i].email}</td>
          <td>${response[i].phonenum}</td>
          <td><a href='#' class='delete' data-id='${response[i]._id}'>Del</a></td><td><a href='#update-contact-container' class='update' data-id='${response[i]._id}' data-role='${response[i].role}' data-name='${response[i].name}' data-email='${response[i].email}' data-phonenum='${response[i].phonenum}'>Update</a></td></tr>`;
    ...
    $("#contact-list").on("click", ".update", function (e) {
      ...
      let contactRole = $(this).data("phonenum")
      let contactName = $(this).data("name");
      let contactEmail = $(this).data("email");
      let contactNum = $(this).data("msg");
      ...
      $("#update-contact-num").val(contactNum);
    ...
    $("#update-contact-submit").on("click", function (e) {
      ...
      let contactNum = $("#update-contact-num").val();
      let contactId = $("#update-contact-id").val();
  
      console.log($("#update-contact-msg").val());
  
      //[STEP 12a]: We call our update form function which makes an AJAX call to our RESTDB to update the selected information
      updateForm(contactRole, contactName, contactEmail, contactNum);
    ...
    function updateForm(id, contactRole, contactName, contactEmail, contactNum) {
      ...
      var jsondata = { "role": contactRole, "name": contactName, "email": contactEmail, "phonenum":contactNum };
      var settings = {
        "async": true,
        "crossDomain": true,
